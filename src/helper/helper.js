...
export const filterAllOrders = (orders) => {
  ...
  var openOrders = _.reject(all, (order) => {
    var filledOrders = filled.some(
      (O) => O.returnValues.id === order.returnValues.id
    );
    var canceledOrder = canceled.some(
      (O) => O.returnValues.id === order.returnValues.id
    );
...
export const markPunchesTag = (order) => {
  const orderType =
    order.returnValues.tokenGive === ETHER_ADDRESS ? 'buy' : 'sell';
  return {
    ...order,
    orderType,
...
export const filterTradeOrders = (orders, account) => {
  ...
  var myTrades = {
    ...
    filledOrders: filledOrders.sort(
      (a, b) => a.returnValues.timestamp - b.returnValues.timestamp
    ),
  };
  return myTrades;
...
export const formatCandleChartData = (orders) => {
  ...
  var [secondLastOrder, lastOrder] = orders.slice(
    orders.length - 2,
    orders.length
  );
...
const createChartData = (orders) => {
  orders = _.groupBy(orders, (o) =>
    moment.unix(o.returnValues.timestamp).startOf('minute').format()
  );
