...
import { ETHER_ADDRESS, formatBalance } from '../helper/helper';
...
export const loadBalances = async (
  ...
  const walletTokenBalance = await tokenContact.methods
    .balanceOf(account)
    .call();
  ...
  const exchangeTokenBalance = await exchangeContract.methods
    .balanceOf(account, tokenContact.options.address)
    .call();
...
export const cancelOrder = async (
  ...
  await exchangeContract.methods
    .cancelOrder(String(order.returnValues.id))
    .send({ from: account })
...
export const fillOrder = async (
  ...
  await exchangeContract.methods
    .fillOrder(String(order.returnValues.id))
    .send({ from: account })
...
export const depositeEther = async (
  web3,
  etherAmount,
  exchangeContract,
  account,
  ...
  await exchangeContract.methods
    .depositeEther()
    .send({ from: account, value: web3.utils.toWei(etherAmount, 'ether') })
    .on('transactionHash', function (receipt) {
      onSuccess();
    })
    .on('error', function (error) {
...
export const withdrawEther = async (
  ...
  await exchangeContract.methods
    .withdrawEther(web3.utils.toWei(etherAmount, 'ether'))
    .send({ from: account })
    .on('transactionHash', function (receipt) {
      onSuccess();
    })
...
export const depositeToken = async (
  web3,
  tokenContract,
  tokenAmount,
  exchangeContract,
  ...
  await tokenContract.methods
    .approve(exchangeContract.options.address, tokenAmount)
    .send({ from: account })
    .on('transactionHash', async function (hash) {
      await exchangeContract.methods
        .depositeToken(tokenContract.options.address, tokenAmount)
        .send({ from: account })
...
export const withdrawToken = async (
  ...
  await exchangeContract.methods
    .withdrawToken(tokenContract.options.address, tokenAmount)
    .send({ from: account })
    .on('transactionHash', function (hash) {
      onSuccess();
    })
